...
public class ConcatenableQueue<E extends Comparable<E>> extends BlackRedTree<E> implements Iterable<E> {
    ...
    //note: for an empty ConcatenableQueue, its minNode and maxNode are undetermined
    ...
    public ConcatenableQueue (Node<E> root, int height, Node<E> minNode, Node<E> maxNode) {
	this.root = root;
	this.height = height;
    ...
    public ConcatenableQueue (ConcatenableQueue<E> other) {
	this.root = null;
	this.height = -1;
	this.minNode = null;
	this.maxNode = null;
    ...
    public Iterator<E> iterator() {
        return new Iterator<E>() {
            private Node<E> current = minNode;
    ...
    public void shallowCopy (ConcatenableQueue<E> other) {
    ...
    public static <E extends Comparable<E>> ConcatenableQueue<E> concatenate (ConcatenableQueue<E> qLeft, ConcatenableQueue<E> qRight) {
	...
	return new ConcatenableQueue<E> (newRoot, newHeight, qLeft.minNode, qRight.maxNode);
    }
    ...
    protected static <E extends Comparable<E>> void splitAt(Node<E> n, int h, E e, ConcatenableQueue<E> qLeft, ConcatenableQueue<E> qRight) {
	...
	if (n.isLeaf) {//base case
	    ...
	    if (e.compareTo(n.lMax.data) == 0) {//another base case
		qLeft.root = n.left;
		qLeft.height = h-1;
	    ...
	    else if (e.compareTo(n.lMax.data) < 0) {//to the left subtree
		if (n.left.color == RED) {
		    n.left.color = BLACK;
    ...
    public ConcatenableQueue<E> split(E e, boolean returnLoR, boolean inclusive) {
	ConcatenableQueue<E> qLeft = new ConcatenableQueue<E>();
	ConcatenableQueue<E> qRight = new ConcatenableQueue<E>();

	//step 0, handle corner cases
	if (root == null) {
	    return qLeft;
	}
	else if (e.compareTo(minNode.data) < 0 || (e.compareTo(minNode.data) == 0 && !inclusive)) {
	    if (returnLoR == RIGHT) {
	...
	else if (e.compareTo(maxNode.data) > 0 || (e.compareTo(maxNode.data) == 0 && inclusive)) {
	    if (returnLoR == RIGHT) {
		return qRight;
	    }
	    else {
	    ...
	    Node<E> itr = root;
	    while (!itr.isLeaf) {
		if (e.compareTo(itr.lMax.data) <= 0) {
		    itr = itr.left;
		}
		else {
		    itr = itr.right;
	    ...
	    else if (e.compareTo(itr.data) < 0) {
		e = itr.left.data;
	    }
	    else {
		e = itr.data;
