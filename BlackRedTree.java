...
public class BlackRedTree<E extends Comparable<E>> implements Tree<E> {
	...
	Node<E> n = new Node<E>(e);//for BlackRedTree, ignore left and right neighbors of this new leaf
	size++;
	return n;
    }
	...
	//for BlackRedTree, do nothing to left and right siblings
	size--;
	...
	if (n.isLeaf) {
	    System.out.println("level " + level + "\t" + n.data.toString());
	    return 1;
	}
	else {
	...
	while (!n.isLeaf) {
	    ...
	    else if (e.compareTo(n.lMax.data) < 0	) {
		n = n.left;
	    }
	    else {
		n = n.right;
